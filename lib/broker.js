...
var Msg = require('bitcore-message');
var bitcore = require('bitcore');
var PublicKey = bitcore.PublicKey;
var Signature = bitcore.crypto.Signature;
var sha256ripemd160 = bitcore.crypto.Hash.sha256ripemd160;
...
messages.CONNECTINFO = function(router, peer, message) {
  ...
  var connectionId = sha256ripemd160(message.toBuffer().slice(1,42));
  ...
  for(var i=0; i<router.endpoints.length; i++) {
    ...
    if(svr.isMatch(pubkey)) {
      var decryptedCredentials = ECIES().privateKey(svr.key).publicKey(message.address).decrypt(credentials);
      var clientPubkey = decryptedCredentials.slice(0, 33);
console.log('==== decryptedCredentials');
console.log(svr.key, svr.key.publicKey);
console.log(pubkey);
console.log(decryptedCredentials);
      if(Msg(Buffer.concat([pubkey.toBuffer(), nonce]).toString())._verify(PublicKey.fromBuffer(clientPubkey), Signature.fromCompact(decryptedCredentials))) {
        ...
        var connectionIdSig = Msg(connectionId.toString())._sign(svr.key);
...
messages.CONNECTACK = function(router, peer, message, connection) {
  ...
  if(Msg(message.connectionId)._verify(request.address, connectionIdSig)) {
